...
void SerialWifiInterface::enable() { 
  ...
  _isEnabled = true;
  clearBuffers();
  WIFI_DEBUG_PRINTLN("WiFi interface enabled");
}

void SerialWifiInterface::disable() {
  _isEnabled = false;
  WIFI_DEBUG_PRINTLN("WiFi interface disabled");
}

size_t SerialWifiInterface::writeFrame(const uint8_t src[], size_t len) {
  if (len > MAX_FRAME_SIZE) {
    WIFI_DEBUG_PRINTLN("writeFrame(), frame too big, len=%d", len);
    return 0;
  }
...
size_t SerialWifiInterface::checkRecvFrame(uint8_t dest[]) {
  ...
  if (newClient) {
    ...
    WIFI_DEBUG_PRINTLN("New WiFi client connected");
  }
    ...
    if (send_queue_len > 0) {   // first, check send queue
      ...
      if(!hasReceivedFrameHeader()){
        ...
        if(client.available() >= frame_header_length){
          ...
          // read frame header
          uint8_t frame_type;
          uint8_t frame_len_bytes[2];
          client.readBytes(&frame_type, 1);
          client.readBytes(frame_len_bytes, 2);
          
          received_frame_header.type = frame_type;
          received_frame_header.length = (frame_len_bytes[0] & 0xFF) | ((frame_len_bytes[1] & 0xFF) << 8);
